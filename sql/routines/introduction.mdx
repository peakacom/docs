...
[function](/sql/routines/function.mdx) block as [inline routines](#inline-routines) or [catalog
...
Refer to the documentation for the [function](/sql/routines/function.mdx) keyword for more
details about declaring the routine overall. The routine body is composed with
statements from the following list:

* [begin](/sql/routines/begin.mdx)
* [case](/sql/routines/case.mdx)
* [declare](/sql/routines/declare.mdx)
...
Statements can also use [built-in functions and operators](/sql/functions/list.mdx) as well
...
documentation](/sql/routines/examples.mdx) for more complex use cases that combine
multiple statements.
...
* Avoid creation of arrays in a looping construct. Each iteration creates a
  ...
  expression](/sql/functions/lambda.mdx) instead of the loop.
