...
Use the `DECLARE` statement directly after the [begin](/sql/routines/begin.mdx) keyword in
[routines](/sql/routines/introduction.mdx) to define one or more variables with an `identifier` as name. 

{/* TODO: Will we add the types file? */}
{/* Each statement must specify the [data type](/sql/reference/types.mdx) of the variable with
...
routines examples documentation](/sql/routines/examples.mdx).
