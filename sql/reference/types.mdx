...
Connectors to data sources are not required to support all Peaka data types
described on this page. If there are data types similar to Peaka's that are used
on the data source, the connector may map the Peaka and remote data types to
each other as needed.

Depending on the connector and the data source, type mapping may apply
in either direction as follows:

- **Data source to Peaka** mapping applies to any operation where columns in the
  data source are read by Peaka, such as a [SELECT](/sql/reference/select) statement, and the
  underlying source data type needs to be represented by a Peaka data type.
...
*IPv4-mapped IPv6 address* range [RFC 4291#section-2.5.5.2](https://datatracker.ietf.org/doc/html/rfc4291.html#section-2.5.5.2`).
