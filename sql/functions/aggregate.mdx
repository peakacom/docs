...
```
Returns an arbitrary non-null value of `x`, if one exists. Identical to [any_value](#any_value).
...
```
See [HyperLogLog Functions](/sql/functions/hyperloglog.mdx).
...
```
See [Quantile digest functions](/sql/functions/qdigest.mdx).
