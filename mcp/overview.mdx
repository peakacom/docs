...
<CardGroup cols={3}>
  <Card title="Remote" href="/mcp/install/remote" icon="cloud">
    Point any client that supports remote MCP at `https://mcp.peaka.studio/mcp`. OAuth handles auth — no API key to copy around. Recommended.
  </Card>
  <Card title="NPM" href="/mcp/install/npm" icon="box">
    Run the server locally via `npx @peaka/mcp-server-peaka@latest`. Authenticates with a Peaka API key.
  </Card>
  <Card title="From source" href="/mcp/install/local" icon="github">
    Clone [`peakacom/peaka-mcp-server`](https://github.com/peakacom/peaka-mcp-server) and run it from a local checkout. For contributors and air-gapped setups.
  </Card>
</CardGroup>
