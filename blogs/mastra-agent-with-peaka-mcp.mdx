...
You can also try the live demo [here](https://peaka-mastra-agent-ui.vercel.app).
...
By default, Mastra agents work well with basic prompting. But I found that explicit instructions help guide the LLM more reliably and add big difference in consistency. To improve the assistants's performance, I manually tested different user questions and tweaked instructions to fix edge cases.
...
![image.png](https://cdn.peaka.com/blogs/mastra-agent/image.png)
...
Then modify your Mastra setup to include deployer:
...
You can access The Peaka assistant-UI finished code on [Github](https://github.com/kobalski/peaka-mastra-agent-ui.git). Follow Readme instructions to run the project.
...
- **Mastra smart interface:** with Mastra powering the logic and assistant-ui providing the frontend, you can build and deploy conversational agents in minutes.
- **Vercel makes it production-ready:** Deploy your agent as a server-less API with zero config and share it instantly online

# Conclusion

In this blog, we walked through how to deploy Peaka AI agent using Mastra framework to Vercel and connect to a user friendly frontend using assistant-ui. Resulting a fully functional chatbot that doesn't just guess but understands the data schema, follows the right SQL syntax, and gives useful feedback even when something goes wrong.

If you are a developer, or just curious about Ai powered interfaces that can retrieve your data, this setup is worth exploring. You can try it, adapt it and build on it without starting from scratch. Feel free to reach out to me on [Github](https://github.com/ramamatar) if you have any questions or comments.
