...
curl --request POST \
  ...
  --data '{
    ...
    "theme": <your_theme_name>,
    "themeOverride": true,
    "connectorType": <"CONNECTOR_MODAL_ONLY" | "FULL_STUDIO">,
...
**ConnectorType** - Controls the UI display mode:

- `CONNECTOR_MODAL_ONLY`: Displays only the connector modal interface, hiding all other UI components for a focused connection experience.
- `FULL_STUDIO`: Shows the complete Peaka UI with all available features and functionality.

**CatalogCreateEnabled** - Boolean flag controlling automatic catalog creation when establishing connections (default: `true`).
